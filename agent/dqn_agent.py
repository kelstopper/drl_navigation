...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        if self.learning_method == LearningMethod.DQN:
        ...
        elif self.learning_method == LearningMethod.DOUBLE_DQN:
            self.qnetwork_local.eval()
            with torch.no_grad():
            ...
            Q_predicted_targets_next = self.qnetwork_local(states).gather(1, actions)

            # Compute loss
            loss = F.mse_loss(Q_predicted_targets_next, Q_targets)
...
class ReplayBuffer:
    ...
    def __init__(self, action_size, buffer_size, batch_size, seed, network=NetworkType.Q_NETWORK):
